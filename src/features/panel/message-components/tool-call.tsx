...
export const AssistantToolCall = memo(function AssistantToolCall({
	...
	const toolLine = (
		<>
			...
			(info.diffAdd != null || info.diffDel != null) ? (
				<span className="flex items-center gap-1 text-[0.786em]">
					{info.diffAdd != null ? (
			...
			{info.command ? (
				<code className="inline-block min-w-0 truncate rounded bg-accent/60 px-1.5 py-0.5 font-mono text-[length:var(--code-font-size,12px)] text-muted-foreground">
					{info.command}
				</code>
			) : info.detail ? (
	...
	if (compact) {
		...
		return (
			<div className="flex max-w-full items-center gap-1.5 py-0.5 text-[0.857em] text-muted-foreground">
				<span className="shrink-0">{info.icon}</span>
	...
	return (
		<>
			...
			>
				<summary
					className={cn(
						"flex max-w-full items-center gap-1.5 py-0.5 text-[0.857em] text-muted-foreground [&::-webkit-details-marker]:hidden",
				...
				{canExpand && isOpen ? (
					<div className="flex flex-col gap-1">
						{hasOutput ? (
							<div className="max-h-[16rem] overflow-auto rounded-md bg-accent/35 text-[length:var(--code-font-size,12px)] leading-[1.55]">
								{info.fullCommand ? (
							...
							<div className="ml-5 flex flex-col gap-0.5 border-l border-border/30 pl-3">
								{info.files!.map((f, i) =>
									f.rawDiff ? (
										...
										<div
											key={`${f.name}-${i}`}
											className="flex max-w-full items-center gap-1.5 rounded-md px-2 py-1 text-[0.857em] leading-4 text-muted-foreground transition-colors hover:bg-accent/60"
										>
											<img
												src={getMaterialFileIcon(f.name)}
												alt=""
											...
											{f.diffAdd != null || f.diffDel != null ? (
												<span className="flex shrink-0 items-center gap-1 text-[0.786em]">
													{f.diffAdd != null ? (
...
const ToolCallErrorRow = memo(function ToolCallErrorRow({
	...
	return (
		...
		>
			<summary
				className={cn(
					"flex max-w-full items-center gap-1.5 py-0.5 text-[0.857em] text-destructive [&::-webkit-details-marker]:hidden",
			...
			>
				...
				{exitCode != null ? (
					<code className="shrink-0 rounded bg-destructive/10 px-1.5 py-0.5 font-mono text-[length:var(--code-font-size,12px)]">
						Exit code {exitCode}
					</code>
				) : null}
				{preview ? (
					<span className="min-w-0 truncate font-mono text-[length:var(--code-font-size,12px)] text-destructive/80">
						{preview}
					</span>
				) : null}
				{expandable ? (
			...
			{expandable && open ? (
				<div className="mt-0.5 max-h-[16rem] overflow-auto rounded-md border border-destructive/15 bg-destructive/[0.05] text-[length:var(--code-font-size,12px)] leading-[1.55]">
	...
	return (
		<div className="flex flex-col">
			<div className="flex max-w-full items-center gap-1.5 py-0.5 text-[0.857em] text-muted-foreground">
				...
				<span className="shrink-0 text-[0.786em] text-muted-foreground/40">
					{toolUseCount > 0
			...
			<div className="ml-5 flex flex-col gap-0.5 border-l border-border/30 pl-3 pt-1">
				{canToggle ? (
					<Button
						...
						className="mb-0.5 h-auto items-center justify-start gap-1 px-0 text-[0.786em] text-muted-foreground/50 hover:bg-transparent hover:text-muted-foreground"
					>
						<ChevronDown
							className={cn(
					...
					{visibleParts.map((part) => {
						...
						if (part.type === "text" && part.text) {
							return (
								<div
									key={key}
									className="text-[0.929em] leading-6 text-muted-foreground"
								>
...
export function CollapsedToolGroup({
	...
	return (
		<details
			...
			open={open}
		>
			<summary className="flex max-w-full cursor-interactive items-center gap-1.5 py-0.5 text-[0.857em] text-muted-foreground [&::-webkit-details-marker]:hidden">
