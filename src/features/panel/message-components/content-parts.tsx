...
export function TodoList({ part }: { part: TodoListPart }) {
	...
	return (
		<div className="my-1 flex flex-col gap-0.5 rounded-md border border-border/40 bg-accent/35 px-3 py-2 text-[0.929em] leading-6 text-muted-foreground">
			<div className="mb-0.5 flex items-center gap-1.5 text-[0.786em] text-muted-foreground">
...
export function PlanReviewCard({ part }: { part: PlanReviewPart }) {
	return (
		<div className="rounded-xl border-[1.5px] border-border/70 bg-background/60 px-3.5 py-3">
			<div className="flex items-center gap-1.5 text-[0.786em] font-medium uppercase tracking-[0.06em] text-muted-foreground">
				<ClipboardList className="size-3.5" strokeWidth={1.8} />
				Plan
			</div>
			{part.planFilePath ? (
				<p className="mt-2 break-words text-[0.857em] leading-5 text-muted-foreground">
					{part.planFilePath}
				</p>
			) : null}
			<div className="conversation-markdown mt-2 max-w-none break-words text-[0.929em] leading-6 text-foreground">
				<Suspense
					fallback={
			...
			{(part.allowedPrompts ?? []).length > 0 ? (
				<div className="mt-3 grid gap-2 rounded-lg border border-border/50 bg-muted/20 p-2.5">
					<p className="text-[0.786em] font-medium uppercase tracking-[0.06em] text-muted-foreground">
						Approved Prompts
					</p>
					{part.allowedPrompts?.map((entry) => (
						...
						>
							...
							<p className="mt-1 whitespace-pre-wrap break-words text-[0.857em] leading-5 text-foreground">
								{entry.prompt}
							</p>
						</div>
					))}
