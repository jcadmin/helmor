	...
	return (
		<div className="my-0.5 flex flex-col">
			<button
				...
				disabled={!expandable}
				className={cn(
					"flex w-full flex-wrap items-center gap-x-1.5 gap-y-0 py-0.5 text-left text-[0.857em] text-muted-foreground",
			...
			>
				...
				{model ? (
					<span className="ml-auto shrink-0 text-[0.786em] text-muted-foreground/40">
						{model}
					</span>
				) : null}
				{glyph}
				...
				<div className="ml-5 mt-1 flex flex-col gap-2 border-l border-border/30 pl-3">
					{prompt ? (
						<div className="flex flex-col gap-1">
							<span className="text-[0.786em] uppercase tracking-wide text-muted-foreground/50">
								Prompt
							</span>
							<div className="whitespace-pre-wrap break-words rounded-md bg-accent/35 px-2.5 py-1.5 text-[0.857em] leading-5 text-muted-foreground/85">
								{prompt}
							</div>
						</div>
					) : null}
					<div className="flex flex-col gap-1">
						...
						{resultText ? (
							...
							<div className="flex items-center gap-1.5 rounded-md bg-accent/20 px-2.5 py-1.5 text-[0.857em] text-muted-foreground/60">
								{live ? (
									<>
										<LoaderCircle
