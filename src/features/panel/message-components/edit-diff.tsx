	...
	return (
		<>
			<span
				...
				className={cn(
					"items-center gap-1.5 text-[0.857em] leading-4 text-muted-foreground transition-colors",
					variant === "row"
			...
			>
				...
				{diffAdd != null || diffDel != null ? (
					<span
						className={cn(
							"flex shrink-0 items-center gap-1 text-[0.786em]",
							variant === "row" ? "" : "ml-auto",
						)}
					>
						{diffAdd != null ? (
			...
			{pos
				? createPortal(
						...
						>
							<div className="shrink-0 border-b border-border/50 px-3 py-1.5 text-[0.786em] text-muted-foreground">
								{file}
							</div>
							<div className="min-h-0 max-h-[24rem] flex-1 overflow-auto font-mono text-[length:var(--code-font-size,12px)] leading-[1.55]">
								{oldStr
									? oldStr.split("\n").map((line, index) => (
											<div
												key={`d${index}`}
