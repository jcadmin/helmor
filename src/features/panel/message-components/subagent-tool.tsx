...
function SpawnAgentRow({
	...
	return (
		<div className="flex flex-col gap-0.5 text-[0.857em]">
			<button
				type="button"
			...
			{prompt ? (
				open ? (
					<div className="ml-5 mt-0.5 whitespace-pre-wrap break-words rounded-md bg-accent/35 px-2.5 py-1.5 text-[0.857em] leading-5 text-muted-foreground/85">
						{prompt}
					</div>
				) : (
	...
	return (
		<div className="my-0.5 flex flex-col">
			<button
				type="button"
				onClick={() => setOpen((v) => !v)}
				className="flex w-fit cursor-interactive items-center gap-1.5 py-0.5 text-left text-[0.857em] text-muted-foreground"
			>
				<Sparkles
	...
	return (
		<div className="my-0.5 flex flex-col">
			<button
				...
				disabled={!hasBodies}
				className={cn(
					"flex w-fit items-center gap-1.5 py-0.5 text-left text-[0.857em] text-muted-foreground",
			...
			{open && hasBodies ? (
				<div className="ml-[7px] mt-1 flex flex-col gap-2 border-l border-border/30 pl-3 text-[0.857em]">
					{states.map((s) => {
						...
						return (
							<div key={s.threadId} className="flex flex-col gap-0.5">
								...
								{s.message ? (
									<div className="ml-5 whitespace-pre-wrap break-words rounded-md bg-accent/35 px-2.5 py-1.5 text-[0.857em] leading-5 text-muted-foreground/85">
										{s.message}
									</div>
								) : null}
							</div>
...
function SubAgentMiscRow({ part }: { part: ToolCallPart }) {
	...
	return (
		<div
			className={cn(
				"my-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0 text-[0.857em]",
				tokens.muted,
			)}
		>
